...
uSpec connects Cursor and Figma into a single pipeline. You provide a component link and context, and the system produces formatted documentation directly in your Figma file.
...
flowchart LR
    ...
    B -->|Extract + Render| C[Console MCP]
    C -->|Documentation frame| D[Figma Output]
```

Every skill extracts component data and renders documentation directly in Figma via the Console MCP. The internal steps differ depending on what each skill needs to analyze. The diagrams below show what happens inside each skill.
...
```mermaid
flowchart TB
    Skill[SKILL.md] --> MCP[Figma Console MCP]
...
flowchart TB
    ...
    Checklist --> Render[Render in Figma via figma_execute]
    Render --> Validate[Screenshot validation]
```
  ...
  <Tab title="Structure">
    The structure skill loads its instruction file, measures all dimensional properties, and renders them into tables by variant axis (size, density, or shape) directly in Figma.
...
flowchart TB
    ...
    Instruction --> Analyze[Analyze structure]
    MCP --> Measure[Measure dimensions and spacing]
    MCP --> VarAxes[Identify variant axes]
    MCP --> TokenVals[Extract token-bound values]

    Measure --> DeepExplore[Deep exploration: walk sub-components]
    VarAxes --> DeepExplore
    DeepExplore --> Organize[Organize by section]
    TokenVals --> Organize

    Analyze --> Columns[Determine column pattern]
    Columns --> Organize
    Organize --> Hierarchy[Apply property hierarchy]
    Hierarchy --> Checklist[Audit against checklist]
    Checklist --> Render[Render in Figma via figma_execute]
    Render --> Validate[Screenshot validation]
```

  After measuring the top-level component, the skill runs a deep exploration step: it walks sub-components, enables all boolean toggles, and inspects internal child trees across sizes and states. Values are reported as token references when bound to a variable (e.g., `sizing-button-lg (56)`) or as plain numbers when hardcoded. The column pattern depends on the component: sizes get columns like Large, Medium, Small; density gets Compact, Default, Spacious.
  </Tab>
...
flowchart TB
    ...
    Instruction --> MCP[Figma Console MCP]
...
flowchart TB
    ...
    subgraph MCP["Figma Console MCP"]
        Bridge[Desktop Bridge]
    end

    Agent <-->|Component data, tokens, styles| Bridge
    Agent -->|Render documentation| Bridge
    Bridge <-->|WebSocket| File
    File --> Output
```

The Figma Console MCP acts as the bridge between Cursor and Figma Desktop. It connects via WebSocket through the Desktop Bridge plugin, giving the agent real-time access to your file data without requiring manual export. Every skill renders through the MCP bridge.
